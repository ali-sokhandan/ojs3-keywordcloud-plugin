...
(function() {
	function wordcloud() {
			...
			text.style("font-family", function(d) {
				...
				.on("mouseover", function(d, i) {
					if (onwordclick !== undefined) {
						d3.select(this).transition().style({'font-size': d.size + 3 + 'px','opacity':'1'});
					}
				})
				.on("mouseout", function(d, i) {
					if (onwordclick !== undefined) {
						d3.select(this).transition().style({'font-size': d.size + 'px','opacity':'0.5'});
					}
				})
		...
		function update() {
			...
			fontSize = d3.scale[scale]().range([12, 32]);
			if (words.length) {
